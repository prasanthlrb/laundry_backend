...
if (Input::has('agent') && !empty(Input::get('agent')))
{
    $q->where('o.agent_id',Input::get('agent'));
}
...
if (Input::has('service') && !empty(Input::get('service')))
{
    ...
    $pdf = PDF::loadView('orderReportpdf', compact('agent','service','customer','data'));
